#!/usr/bin/env python3
"""Thin bootstrap boundary validator for Foundry session-import fixture output.

Validates ONLY mechanical file-existence, JSON parse, schema_version, and
external-writes safety. Does NOT evaluate extractor pass/fail, example
relevance, or promotion-dossier prose — those belong to Foundry.
"""
...
def main() -> int:
    if len(sys.argv) != 2:
        print(
            "Usage: validate_foundry_session_import_fixture.py <output-dir>",
            file=sys.stderr,
        )
        return 2
    ...
    # 6. eval_examples.json must have total_examples >= 1
