# HERMES BOOTSTRAP FLAKE
# =======================
# Standard NixOS flake (no flake-parts dependency).
# Deploy: sudo nixos-rebuild switch --flake /path/to/this#hermes
{
  description = "Hermes OS — Self-owned AI environment on NixOS";

  # ─── INPUTS ────────────────────────────────────────────────────────────
  inputs = {
    nixpkgs.url = "github:NixOS/nixpkgs/nixos-24.05";

    # The real hermes-agent from NousResearch.
    # Keep its own nixpkgs input: the upstream package currently needs packages
    # such as `tirith` that are present in nixos-unstable but absent from
    # nixos-24.05. Forcing it to follow this system's stable nixpkgs makes the
    # upstream package fail during full NixOS toplevel evaluation.
    hermes-agent.url = "github:NousResearch/hermes-agent";
  };

  # ─── OUTPUTS ───────────────────────────────────────────────────────────
  outputs = inputs@{ self, nixpkgs, hermes-agent, ... }:
    let
      deployment = import ./deployment-options.nix;
    in
    {
      # Build the hermes-agent package
      packages.x86_64-linux.default =
        hermes-agent.packages.x86_64-linux.default;

      # NixOS system configuration
      nixosConfigurations.hermes = nixpkgs.lib.nixosSystem {
        system = "x86_64-linux";

        modules = [
          # 1. Import hermes-agent's own NixOS module (the real deal)
          hermes-agent.nixosModules.default

          # 2. Hardware config (generated by nixos-generate-config on target)
          ./hardware-configuration.nix

          # 2b. Deployment-time filesystem override. The live installer writes
          # this from the known internal target partitions after mounting them,
          # preventing nixos-generate-config from accidentally preserving USB
          # media UUIDs such as the NIXOS-BOOT stick for /boot/efi.
          ./hermes-target-filesystems.nix

          # 2c. Phase 1 local observability harness. This is systemd-owned and
          # deliberately independent from Hermes cron, Kanban, messaging, and LLMs.
          ./harness.nix

          # 3. Hermes OS configuration
          ({ config, pkgs, lib, ... }:

          {
            # ─────────────────────────────────────────────────────────────
            # WHO AM I
            # ─────────────────────────────────────────────────────────────
            services.hermes-agent = {
              enable = true;

              user = deployment.agentUser;
              group = deployment.agentGroup;
              createUser = true;
              stateDir = deployment.stateDir;
              workingDirectory = deployment.workspaceDir;

              # Runtime mode is deployment-controlled. Native mode is the default
              # for first boot because it uses the Nix-built package and avoids
              # Docker Hub, apt, NodeSource, Astral uv, and uv Python downloads
              # during the installed system's first service start. Enable
              # containerMode in deployment-options.nix after provisioning a
              # network/cache path for the writable OCI tool layer.
              container.enable = deployment.containerMode;
              container.backend = deployment.containerBackend;
              container.image = deployment.containerImage;
              addToSystemPackages = true;

              # ─────────────────────────────────────────────────────────────
              # LLM PROVIDER
              # ─────────────────────────────────────────────────────────────
              settings = {
                model = {
                  provider = deployment.provider;
                  default = deployment.model;
                };
                toolsets = [ "all" ];
                terminal.backend = "local";
                terminal.cwd = deployment.workspaceDir;
                compression.enabled = true;
                compression.threshold = 0.85;
                gateway.host = deployment.gatewayHost;
                gateway.port = deployment.gatewayPort;
                logging.level = "INFO";
              };

              # API keys via environment file (seeded from USB or set post-install)
              environmentFiles = [ deployment.secretsEnvFile ];

              environment = {
                HERMES_MANAGED = "true";
                HERMES_NODE_NAME = deployment.nodeName;
              };

              # ─────────────────────────────────────────────────────────────
              # SEED DOCUMENTS — placed in workspace on first deploy
              # ─────────────────────────────────────────────────────────────
              documents = {
                "SOUL.md" = ''
                  You are Hermes OS — a fully autonomous AI agent
                  running on your own hardware, your own infrastructure.

                  You own your environment completely. You can read and modify
                  every configuration file, every service, every tool.

                  Your purpose: work autonomously on any task, using your full
                  suite of skills, memory, and companions.

                  You are not a chatbot. You are an agent.
                '';

                "OPERATIONS.md" = ''
                  # Hermes OS — Operational Reference

                  ## System Paths
                  - State:  /var/lib/hermes/.hermes
                  - Config: /var/lib/hermes/.hermes/config.yaml
                  - Logs:   journalctl -u hermes-agent -f
                  - Memory:  /var/lib/hermes/.hermes/memories/
                  - Sessions: /var/lib/hermes/.hermes/sessions/
                  - Workspace: /var/lib/hermes/workspace/

                  ## Common Commands
                  - hermes status          # CLI status
                  - hermes-agent --version
                  - systemctl status hermes-agent
                  - journalctl -u hermes-agent -n 100 --no-pager

                  ## Self-Modification
                  - NixOS config: /etc/nixos/configuration.nix
                  - Rebuild:      sudo nixos-rebuild switch --flake /etc/nixos
                  - hermes config (YAML): /var/lib/hermes/.hermes/config.yaml
                  - Restart after config change: systemctl restart hermes-agent

                  ## Backup
                  - Wiki: cd /var/lib/hermes/wiki && git push
                  - Skills: cd /var/lib/hermes/skills && git push
                  - Dotfiles: /etc/nixos is a git repo

                  ## Update hermes-agent
                  - Update flake lock: cd /etc/nixos && sudo nix flake lock --update-input hermes-agent
                  - Rebuild: sudo nixos-rebuild switch --flake /etc/nixos
                  - Restart: systemctl restart hermes-agent
                '';
              };
            };

            # ─────────────────────────────────────────────────────────────
            # EXTRA PACKAGES available to the agent and interactive shell
            # ─────────────────────────────────────────────────────────────
            services.hermes-agent.extraPackages = import ./agent-extra-packages.nix { inherit pkgs; };

            # Keep Hermes state operator-readable from the console admin user.
            # The service runs as deployment.agentUser while live debugging runs
            # `hermes ...` as deployment.adminUser. Without group-readable state,
            # commands such as `hermes status` can crash on files created by the
            # service, e.g. /var/lib/hermes/.hermes/cron/jobs.json.
            systemd.services.hermes-agent.serviceConfig = {
              UMask = "0007";
              # The upstream NixOS module still exports MESSAGING_CWD for
              # workingDirectory, but Hermes now wants terminal.cwd in config.yaml.
              UnsetEnvironment = [ "MESSAGING_CWD" ];
              # Hermes' cron scheduler currently creates ~/.hermes/cron with 0700
              # on service start. Keep console admin access group-readable after
              # every restart so `hermes status` does not hit PermissionError.
              ExecStartPost = "+${pkgs.writeShellScript "hermes-agent-state-permissions-poststart" ''
                set -euo pipefail
                # The gateway creates cron state shortly after process start.
                # Wait briefly so this runs after those mkdir(..., 0700) calls.
                sleep 5
                if [ -d ${deployment.stateDir}/.hermes ]; then
                  chgrp -R ${deployment.agentGroup} ${deployment.stateDir}/.hermes || true
                  find ${deployment.stateDir}/.hermes -type d -exec chmod 2770 {} + || true
                  find ${deployment.stateDir}/.hermes -type f -exec chmod g+rw,o-rwx {} + || true
                fi
              ''}";
            };
            system.activationScripts.hermesAdminStateAccess.text = ''
              if [ -d ${deployment.stateDir}/.hermes ]; then
                chgrp -R ${deployment.agentGroup} ${deployment.stateDir}/.hermes || true
                find ${deployment.stateDir}/.hermes -type d -exec chmod 2770 {} + || true
                find ${deployment.stateDir}/.hermes -type f -exec chmod g+rw,o-rwx {} + || true
              fi
            '';

            # ─────────────────────────────────────────────────────────────
            # INTERACTIVE ADMIN USER
            # ─────────────────────────────────────────────────────────────
            users.users.${deployment.adminUser} = {
              isNormalUser = true;
              description = deployment.adminDescription;
              extraGroups = [ deployment.agentGroup "docker" "wheel" "networkmanager" ];
              home = "/home/${deployment.adminUser}";
              createHome = true;
              shell = pkgs.bashInteractive;
              openssh.authorizedKeys.keys = deployment.adminAuthorizedKeys;
            } // lib.optionalAttrs (deployment.adminInitialPassword != null) {
              initialPassword = deployment.adminInitialPassword;
            };

            services.getty.autologinUser = lib.mkIf deployment.consoleAutologin deployment.adminUser;

            # ─────────────────────────────────────────────────────────────
            # NETWORK
            # ─────────────────────────────────────────────────────────────
            networking.hostName = deployment.hostName;
            # NetworkManager gives console admins nmcli/nmtui for Wi-Fi setup.
            # This is intentionally enabled even for headless nodes because some
            # repair locations have no ethernet path to the router.
            networking.useDHCP = lib.mkDefault false;
            networking.networkmanager.enable = true;
            environment.systemPackages = [
              pkgs.networkmanager
              pkgs.wpa_supplicant
              pkgs.iw
            ];

            services.openssh = {
              enable = true;
              settings = {
                PermitRootLogin = "no";
                PasswordAuthentication = false;
              };
            };

            # ─────────────────────────────────────────────────────────────
            # BOOT (x86_64 UEFI)
            # ─────────────────────────────────────────────────────────────
            boot.loader.grub.enable = lib.mkForce false;
            boot.loader.systemd-boot.enable = true;
            boot.loader.efi.canTouchEfiVariables = true;
            boot.loader.efi.efiSysMountPoint = "/boot/efi";
            boot.initrd.availableKernelModules = [
              "ahci" "xhci_pci" "usb_storage" "nvme"
              "ext4" "vfat" "sr_mod" "sd_mod"
            ];

            # ─────────────────────────────────────────────────────────────
            # LOCALE / TIME
            # ─────────────────────────────────────────────────────────────
            time.timeZone = deployment.timeZone;
            i18n.defaultLocale = deployment.locale;

            # ─────────────────────────────────────────────────────────────
            # NIX
            # ─────────────────────────────────────────────────────────────
            system.stateVersion = "24.05";
            nix.settings.experimental-features = [ "nix-command" "flakes" ];
            nix.settings.accept-flake-config = true;
            nix.gc.automatic = true;
            nix.gc.dates = "weekly";
            nix.gc.options = "--delete-older-than 30d";

            # ─────────────────────────────────────────────────────────────
            # SECURITY
            # ─────────────────────────────────────────────────────────────
            security.sudo.enable = true;
            security.sudo.wheelNeedsPassword = !deployment.passwordlessSudo;
            boot.kernel.sysctl."kernel.randomize_va_space" = 2;

            # ─────────────────────────────────────────────────────────────
            # DOCUMENTATION (minimal — save space, docs in wiki)
            # ─────────────────────────────────────────────────────────────
            documentation.enable = false;
            documentation.man.enable = false;

            # ─────────────────────────────────────────────────────────────
            # VIRTUALISATION (Docker)
            # ─────────────────────────────────────────────────────────────
            virtualisation.docker.enable = true;

            systemd.services.hermes-container-image-preload = lib.mkIf deployment.containerMode {
              description = "Preload Hermes Agent container images before service start";
              before = [ "hermes-agent.service" ];
              after = [ "local-fs.target" ]
                ++ lib.optional (deployment.containerBackend == "docker") "docker.service"
                ++ lib.optional (deployment.containerBackend == "podman") "podman.service";
              requires = lib.optional (deployment.containerBackend == "docker") "docker.service"
                ++ lib.optional (deployment.containerBackend == "podman") "podman.service";
              wantedBy = [ "multi-user.target" ];
              serviceConfig = {
                Type = "oneshot";
                RemainAfterExit = true;
                ExecStart = let
                  preloadScript = pkgs.writeShellScript "hermes-container-image-preload" ''
                    set -euo pipefail
                    archive_dir=${lib.escapeShellArg deployment.containerImageArchiveDir}
                    backend=${lib.escapeShellArg deployment.containerBackend}
                    image=${lib.escapeShellArg deployment.containerImage}

                    if [[ ! -d "$archive_dir" ]]; then
                      echo "No Hermes container image archive directory: $archive_dir"
                      exit 0
                    fi

                    shopt -s nullglob
                    archives=("$archive_dir"/*.tar "$archive_dir"/*.tar.gz "$archive_dir"/*.oci)
                    if (( ''${#archives[@]} == 0 )); then
                      echo "No Hermes container image archives found in $archive_dir"
                      echo "Cold containerMode=true startup may pull $image and run apt/NodeSource/Astral/uv provisioning."
                      exit 0
                    fi

                    case "$backend" in
                      docker)
                        command -v docker >/dev/null || { echo "docker command missing" >&2; exit 1; }
                        for archive in "''${archives[@]}"; do
                          echo "Loading Hermes container image archive with docker: $archive"
                          docker load -i "$archive"
                        done
                        ;;
                      podman)
                        command -v podman >/dev/null || { echo "podman command missing" >&2; exit 1; }
                        for archive in "''${archives[@]}"; do
                          echo "Loading Hermes container image archive with podman: $archive"
                          podman load -i "$archive"
                        done
                        ;;
                      *)
                        echo "Unsupported Hermes container backend: $backend" >&2
                        exit 1
                        ;;
                    esac
                  '';
                in "${preloadScript}";
              };
            };

            # ─────────────────────────────────────────────────────────────
            # GATEWAY SECURITY — bind to localhost only.
            # Safe for LAN-isolated host. For reverse-proxy deployment,
            # bind to 127.0.0.1 and terminate TLS at the proxy.
            # ─────────────────────────────────────────────────────────────
            # Gateway host/port are configured in deployment-options.nix.
          })
        ];
      };
    };
}
