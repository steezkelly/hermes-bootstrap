...
from test_foundry_boundary_validation import (
    ...
    test_boundary_validator_service_is_manual_default_off as ar_validator_off,
    test_boundary_validator_service_no_network as ar_validator_nonet,
    test_boundary_validator_service_no_github as ar_validator_nogh,
    test_boundary_validator_service_is_readonly as ar_validator_ro,
)
...
VALIDATOR_PY = REPO_ROOT / "scripts" / "harness" / "validate_foundry_tool_underuse_fixture.py"
...
def test_tool_underuse_wrapper_nix_binding_exists():
    text = _harness_text()
    assert "foundryToolUnderuseFixture" in text
    assert 'name = "hermes-evolution-foundry-tool-underuse-fixture"' in text


def test_tool_underuse_wrapper_nix_binding_calls_python():
    text = _harness_text()
    assert "evolution.core.tool_underuse_demo" in text
    assert "--mode fixture" in text
    assert "--no-network" in text
    assert "--no-external-writes" in text
    assert "/var/lib/hermes/reports/evolution/tool-underuse-fixture" in text


def test_tool_underuse_wrapper_service_is_manual_default_off():
    text = _harness_text()
    svc = "systemd.services.hermes-evolution-foundry-tool-underuse-fixture"
    tmr = "systemd.timers.hermes-evolution-foundry-tool-underuse-fixture"
    assert svc in text
...
def test_tool_underuse_wrapper_service_safe():
    text = _harness_text()
    svc = "systemd.services.hermes-evolution-foundry-tool-underuse-fixture"
    svc_start = text.index(svc)
    svc_end = text.find("systemd.", svc_start + 1)
    ...
    assert "ReadOnlyPaths" in block and "/var/lib/hermes/foundry" in block
    assert "InaccessiblePaths" in block
    assert "EnvironmentFile" not in block


def test_tool_underuse_wrapper_service_no_network():
    text = _harness_text()
    svc = "systemd.services.hermes-evolution-foundry-tool-underuse-fixture"
    svc_start = text.index(svc)
    svc_end = text.find("systemd.", svc_start + 1)
    block = text[svc_start:] if svc_end == -1 else text[svc_start:svc_end]
    assert "http" not in block and "github.com" not in block


def test_tool_underuse_wrapper_service_no_github():
    text = _harness_text()
    svc = "systemd.services.hermes-evolution-foundry-tool-underuse-fixture"
    svc_start = text.index(svc)
    svc_end = text.find("systemd.", svc_start + 1)
    block = text[svc_start:] if svc_end == -1 else text[svc_start:svc_end]
    assert "EnvironmentFile" not in block and "GITHUB_TOKEN" not in block
...
def test_tool_underuse_validator_nix_binding_exists():
    text = _harness_text()
    assert "validateFoundryToolUnderuseFixture" in text
    assert 'name = "hermes-validate-foundry-tool-underuse-fixture"' in text


def test_tool_underuse_validator_python_script_exists():
    assert VALIDATOR_PY.is_file()


def test_tool_underuse_validator_service_is_manual_default_off():
    text = _harness_text()
    svc = "systemd.services.hermes-validate-foundry-tool-underuse-fixture"
    tmr = "systemd.timers.hermes-validate-foundry-tool-underuse-fixture"
    assert svc in text
    assert tmr not in text
    svc_start = text.index(svc)
    svc_end = text.find("systemd.", svc_start + 1)
...
def test_tool_underuse_validator_checks_expected_files():
    vtext = _validator_text()
    assert "run_report.json" in vtext
    assert "tool_usage_snapshot.json" in vtext
    assert "promotion_dossier.md" in vtext
    assert "artifact_manifest.json" in vtext


def test_tool_underuse_validator_service_no_network():
    text = _harness_text()
    svc = "systemd.services.hermes-validate-foundry-tool-underuse-fixture"
    svc_start = text.index(svc)
    svc_end = text.find("systemd.", svc_start + 1)
...
def test_tool_underuse_validator_service_no_github():
    text = _harness_text()
    svc = "systemd.services.hermes-validate-foundry-tool-underuse-fixture"
    svc_start = text.index(svc)
    svc_end = text.find("systemd.", svc_start + 1)
...
def test_tool_underuse_validator_service_is_readonly():
    text = _harness_text()
    svc = "systemd.services.hermes-validate-foundry-tool-underuse-fixture"
    svc_start = text.index(svc)
    svc_end = text.find("systemd.", svc_start + 1)
